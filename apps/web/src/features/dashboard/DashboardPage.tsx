    ...
    <div className="pb-4">
      ...
      >
        ...
        <div className="relative">
          ...
          <h1 className="mt-1 text-h1 text-text-on-brand sm:mt-2 sm:text-display">
            Olá, {firstName}
          </h1>
          {summary && !isEmpty ? (
            <div className="mt-4 flex flex-wrap gap-2 sm:mt-6 sm:gap-2">
              {overdueCount > 0 && (
                <span className="inline-flex items-center gap-2 rounded-full bg-[var(--danger)] px-4 py-1 text-caption font-semibold text-white shadow-sm sm:px-4 sm:py-2 sm:text-body-sm">
              ...
              <HeroChip
                ...
                label={
                  summary.today.dueToday.length + summary.today.overdue.length === 1
                    ? "tarefa hoje"
                    : "tarefas hoje"
                }
              />
              ...
              <HeroChip
                ...
                label={summary.metrics.openNegotiations === 1 ? "negociação" : "negociações"}
              />
            </div>
          ) : (
            <p className="mt-4 max-w-md text-body text-[var(--brand-navy-100)]">
      ...
      {/* Primeira dobra depois do herói: o que veio de fora. Sinal de cliente
          esperando resposta manda mais que qualquer número do mês. */}
      <ClientSignals />
...
function DashboardContent({ summary }: { summary: DashboardSummary }) {
  ...
  return (
    <div className="mt-8 flex flex-col gap-12">
      {/* Trabalho do dia primeiro. Número do mês é leitura; tarefa e alerta
          são ação, e ação ganha a dobra. */}
      <div className="grid items-start gap-12 lg:grid-cols-2 lg:gap-8">
        <Section id="hoje" title="Hoje" caption="suas tarefas do dia" delay={60}>
          {tasks.length > 0 ? (
            <GroupCard>
        ...
        {visibleAlerts.length > 0 && (
          <Section title="Radar" caption="o que precisa de atenção" delay={120}>
            <GroupCard>
        ...
        <div className="grid overflow-hidden rounded-2xl border border-border bg-surface shadow-sm sm:grid-cols-3 sm:divide-x sm:divide-border">
          ...
          <div className="flex items-center gap-4 border-t border-border p-6 sm:border-t-0">
            ...
            <div>
              ...
              <div className="mt-1 text-body-sm text-text-muted">Tempo médio até fechar</div>
            </div>
          </div>
        </div>
      </Section>
...
function HeroChip({ value, label }: { value: number; label: string }) {
  return (
    <span className="inline-flex items-baseline gap-2 rounded-full bg-[rgba(255,255,255,0.12)] px-4 py-1 text-caption text-[var(--brand-navy-100)] sm:px-4 sm:py-2 sm:text-body-sm">
...
function AlertRow({
  ...
  return (
    <Link
      to={to}
      className="group flex items-center gap-4 px-4 py-4 transition-colors duration-fast hover:bg-surface-sunken focus-visible:bg-surface-sunken sm:px-6"
    >
      <span
        className={
...
function TrendChip({ value, previous }: { value: number; previous: number }) {
  if (value === previous) {
    return (
      <span className="inline-flex items-center gap-1 rounded-full bg-surface-sunken px-2 py-1 text-caption font-bold text-text-muted">
        estável
      </span>
    );
  }
...
function LeadsChart({ series }: { series: MonthCount[] }) {
  ...
  return (
    <div className="mt-6">
      <div className="flex h-24 items-end gap-2 sm:h-32 sm:gap-4">
        {series.map((m, i) => {
...
function FunnelBar({
  ...
  return (
    <div className="flex items-center gap-4">
      {/* Etapa em duas linhas quando precisar: rótulo cortado esconde o dado. */}
      <span className="w-24 flex-none text-body-sm leading-snug text-text-muted sm:w-32">
...
function TaskRow({ task, overdue = false }: { task: DashboardTask; overdue?: boolean }) {
  return (
    ...
    >
      <span
        className={
          "grid w-[4.25rem] flex-none place-items-center rounded-lg px-2 py-2 text-caption font-bold tabular-nums " +
...
function DashboardSkeleton() {
  return (
    <div className="mt-8 flex animate-pulse flex-col gap-10" aria-hidden="true">
